...
export function faqPage(faqs, groups, delivery) {
...
export function buildGraph({ site, business, faqs, delivery, page, schema }) {
  ...
  if (schema.faqs) graph.push(faqPage(faqs, schema.faqs, delivery));
  ...
  if (schema.store) {
    ...
    graph.push({ "@id": pageUrl + "#store", ...liquorStore(site, business, store, pageUrl) });
  }
