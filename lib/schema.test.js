...
test("FAQPage text matches faqs.js and honors the confirmed-terms guard", () => {
  const unconfirmed = { ...delivery, terms: { ...delivery.terms, freeOver: { ...delivery.terms.freeOver, confirmed: false } } };
  ...
  const confirmed = { ...delivery, terms: { ...delivery.terms, freeOver: { ...delivery.terms.freeOver, confirmed: true } } };
  ...
  assert.ok(cost2.acceptedAnswer.text.includes("$50"));
});
