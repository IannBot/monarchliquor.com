...
export function getStatus(hours, now = new Date(), timeZone = "America/Chicago") {
  ...
  const opens = next ? `opens ${fmt12(next.open)} ${whenLabel(next.offset, next.weekday)}` : "";
  let detail = opens;
  if (today.closed && today.holiday) detail = opens ? `${today.label} · ${opens}` : today.label;
  else if (today.closed) detail = opens ? `${DAY_NAMES[weekday]}s · ${opens}` : `${DAY_NAMES[weekday]}s`;
  return { ...base, isOpen: false, status: "Closed", detail, reason: today.closed ? today.label || DAY_NAMES[weekday] : null };
}
...
/**
 * Texas package stores must close on Thanksgiving Day, Christmas Day, and
 * New Year's Day; when Christmas or New Year's falls on a Sunday the store is
 * also closed the following Monday. Returns closure entries for a year.
 */
export function statutoryHolidays(year) {
  const iso = (y, m, d) => `${y}-${String(m).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
  const dow = (y, m, d) => new Date(Date.UTC(y, m - 1, d)).getUTCDay();
  // Thanksgiving: fourth Thursday of November.
  let day = 1;
  while (dow(year, 11, day) !== 4) day++;
  const thanksgiving = day + 21;
  const out = [
    { date: iso(year, 1, 1), label: "New Year's Day", closed: true, statutory: true },
    { date: iso(year, 11, thanksgiving), label: "Thanksgiving Day", closed: true, statutory: true },
    { date: iso(year, 12, 25), label: "Christmas Day", closed: true, statutory: true },
  ];
  if (dow(year, 12, 25) === 0) out.push({ date: iso(year, 12, 26), label: "Day after Christmas (Texas law)", closed: true, statutory: true });
  if (dow(year, 1, 1) === 0) out.push({ date: iso(year, 1, 2), label: "Day after New Year's (Texas law)", closed: true, statutory: true });
