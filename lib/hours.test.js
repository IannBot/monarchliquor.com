...
import { getStatus, weeklySummary, weeklyRows, fmt12, upcomingHolidays } from "./hours.js";
...
test("Sunday is closed all day", () => {
  ...
  assert.equal(s.isOpen, false);
  assert.equal(s.status, "Closed today");
...
test("holiday closure overrides weekly hours", () => {
  ...
  assert.equal(s.isOpen, false);
  assert.equal(s.status, "Closed for Christmas Day");
  assert.equal(s.detail, "opens 10am tomorrow");
});
